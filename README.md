# Escola Gota de Mel — site institucional

Atualizações desta versão:
- hero inicial com ícones ilustrados de educação no lugar das fotos laterais
- remoção do destaque rosa da frase principal, mantendo cores da escola
- ajuste de proporção da seção Nosso Espaço
- foto do pátio em horizontal, mas sem ocupar a tela inteira
- imagens da seção Atividades sem títulos individuais
- lista de atividades organizada em 3 colunas
- imagens de vivências mantidas apenas como exemplos visuais


## Mobile
- menu compacto e toque facilitado
- hero reorganizada para telas pequenas
- galeria e atividades responsivas
- WhatsApp com botão circular no celular
- mapa e espaçamentos otimizados
- suporte a safe areas em iPhone


## Ajustes adicionais
- seção Nosso Espaço subida na ordem da página
- seção Atividades subida e intercalada com blocos de texto
- redução da mancha amarela no mobile
